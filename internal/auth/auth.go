...
func (a Auth) GenerateToken(ctx context.Context, data GenerateToken) (string, error) {
	...
	claims["exp"] = time.Now().Add(96 * time.Hour).Unix()
...
func (a Auth) IsValidToken(ctx context.Context, tokenStr string) (entity.User, error) {
	...
	query := fmt.Sprintf(`SELECT id, username, password, role, status FROM users WHERE username = '%s' AND deleted_at IS NULL`, claims.Username)
	...
	for rows.Next() {
		if err = rows.Scan(&detail.Id, &detail.Username, &detail.Password, &detail.Role, &detail.Status); err != nil {
			return entity.User{}, err
		}
	}
